...
import me.ksanstone.wavesync.wavesync.gui.window.jni.NativeUtilities;
...
public class CustomizedStage {
    ...
    private final boolean isRootReplaced = true;
    ...
    private void snatchControls() {
        var a = stage.getScene().getRoot();
        if (a instanceof MainControl) {
            mainControl = ((MainControl) a);
            mainControl.setup();
            closeButton = mainControl.getControls().getCloseButton();
            restoreButton = mainControl.getControls().getMaximiseButton();
            minimizeButton = mainControl.getControls().getMinimizeButton();
            config.setCaptionDragRegion(mainControl.getDragRegion());
        }
    }
    ...
    class WndProc implements WinUser.WindowProc {
        ...
        private WinDef.LRESULT onWmNcHitTest(WinDef.HWND hWnd, int msg, WinDef.WPARAM wParam, WinDef.LPARAM lParam) {
            ...
            DragRegion captionBounds = config.getDragRegion();
